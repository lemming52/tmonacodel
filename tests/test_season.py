...
from tmonacodel.season import simulate_season
...
class TestSimulateSeason:
    def test_output_shape(self, default_config, precomputed):
        ...
        result = simulate_season(default_config, pts_table, lookup, rng)
    ...
    def test_best_of_rule_bounds(self, default_config, precomputed):
        ...
        rng = np.random.default_rng(0)
        result = simulate_season(default_config, pts_table, lookup, rng)
        # Max possible = best_of * max single-match score
        max_per_match = int(pts_table.max())
        assert int(result.max()) <= default_config.best_of * max_per_match

    def test_small_config_reproducible(self, small_config, small_precomputed):
        pts_table, lookup = small_precomputed
        rng1 = np.random.default_rng(7)
        rng2 = np.random.default_rng(7)
        r1 = simulate_season(small_config, pts_table, lookup, rng1)
        r2 = simulate_season(small_config, pts_table, lookup, rng2)
    ...
    def test_best_of_selects_top_scores(self, small_config, small_precomputed):
        ...
        If a player scored [100, 0, 50, 200] over 4 matches and best_of=2,
        their season total should be 100+200=300.
        This test uses a controlled scenario.
        """
        pts_table, lookup = small_precomputed
        rng = np.random.default_rng(0)
        # Run many seasons and verify totals are non-negative and bounded
        for _ in range(10):
            result = simulate_season(small_config, pts_table, lookup, rng)
            assert np.all(result >= 0)
